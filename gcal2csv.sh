...
HEADER="DTSTART${DELIMITER}DTEND${DELIMITER}DTSTAMP${DELIMITER}UID${DELIMITER}CREATED${DELIMITER}DESCRIPTION${DELIMITER}LAST_MODIFIED${DELIMITER}LOCATION${DELIMITER}SEQUENCE${DELIMITER}STATUS${DELIMITER}SUMMARY${DELIMITER}TRANSP"
...
for line in $CONTENT ; do
  ...
  if [[ "$nline" == STATUS* ]] ; then
    STATUS=`echo $line | sed -e 's/^.*://'`
    CSVLINE="${CSVLINE}${DELIMITER}${STATUS}"
  fi
